...
public class MsService {
    ...
    public String createMsPath(User user, long goodsId) {
        if(user == null || goodsId <= 0) {
            return null;
        }
        String str = MD5Util.md5(UUIDUtil.uuid() + "123456");
        redisService.set(MsKey.getMsPath, ""+user.getId()+"_"+goodsId, str);
        return str;
    }

    public BufferedImage createMsVerifyCode(User user, long goodsId) {
        if(user == null || goodsId <= 0) {
            return null;
        }
        int width = 80;
        ...
        redisService.set(MsKey.getMsVerifyCode, user.getId()+","+goodsId, rnd);
        //输出图片
        return image;
    }
    ...
    public boolean checkVerifyCode(User user, long goodsId, int verifyCode) {
        ...
        Integer codeOld = redisService.get(MsKey.getMsVerifyCode, user.getId()+","+goodsId, Integer.class);
        if(codeOld == null || codeOld - verifyCode != 0 ) {
            return false;
        }
        redisService.delete(MsKey.getMsVerifyCode, user.getId()+","+goodsId);
        return true;
    }
